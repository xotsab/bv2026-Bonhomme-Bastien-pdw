 ...
 */
const COOKIE_DOMAIN_REGEXP = /^\.?[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?)*$/i;
/**
...
export function serializeCookie(name, value, options = {}) {
    ...
    try {
        ...
        throw new TypeError(`Invalid value for cookie "${name}": not a well-formed string`);
    }
    ...
    if (options.maxAge !== undefined) {
        if (!Number.isSafeInteger(options.maxAge)) {
            throw new TypeError(`Invalid maxAge for cookie "${name}": expected an integer number of seconds`);
        }
    ...
    if (options.partitioned) {
        if (!options.secure) {
            throw new TypeError(`Invalid options for cookie "${name}": "partitioned" requires "secure: true"`);
        }
        header += '; Partitioned';
    }
    if (options.priority !== undefined) {
        const priority = PRIORITY_VALUES.get(String(options.priority).toLowerCase());
        if (!priority) {
    ...
    if (options.sameSite !== undefined) {
        const sameSite = SAME_SITE_VALUES.get(String(options.sameSite).toLowerCase());
        if (!sameSite) {
            throw new TypeError(`Invalid sameSite for cookie "${name}"`);
        }
        if (sameSite === 'None' && !options.secure) {
            throw new TypeError(`Invalid options for cookie "${name}": "sameSite: 'none'" requires "secure: true"`);
        }
