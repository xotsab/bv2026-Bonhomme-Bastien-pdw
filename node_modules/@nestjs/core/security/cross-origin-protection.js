...
import { addLeadingSlash, isFunction, isPlainObject, isString, } from '@nestjs/common/internal';
 ...
 */
export function evaluateCrossOriginRequest(request, exemptions = {}) {
...
function exemptOr(request, exemptions, reason) {
 ...
 */
export function normalizeTrustedOrigin(origin) {
    const fail = (reason) => {
    ...
    if (!/^[a-z][a-z0-9+.-]*:\/\/[^/?#\\]+$/i.test(origin)) {
        return fail('expected "scheme://host[:port]", without a path, query string or ' +
            'fragment');
    }
    let url;
    try {
...
const DENIED_MESSAGES = {
    'cross-origin-sec-fetch-site': 'Cross-origin request detected from Sec-Fetch-Site header',
    'cross-origin-origin-header': 'Cross-origin request detected, and/or browser is out of date: ' +
...
export class CrossOriginProtection {
    ...
    constructor(httpAdapter, options = {}) {
        ...
        if (!isPlainObject(options)) {
            throw new Error('Invalid CSRF protection options: expected an options object.');
        }
        for (const key of Object.keys(options)) {
            if (key !== 'trustedOrigins' && key !== 'exclude') {
                throw new Error(`Invalid CSRF protection options: unknown option "${key}" ` +
                    '(expected "trustedOrigins", "exclude").');
            }
        }
        const { trustedOrigins = [] } = options;
        if (!Array.isArray(trustedOrigins)) {
            throw new Error('Invalid CSRF protection "trustedOrigins" option: expected a list ' +
                'of origins.');
        }
        ...
        if (isFunction(exclude)) {
            ...
            throw new Error('Invalid CSRF protection "exclude" option: expected a list of ' +
     ...
     */
    resolveExclusions(pathExtractor) {
        this.resolvedExclusions = this.excludedRoutes.flatMap(route => pathExtractor
    ...
    isExcluded(request) {
        ...
        return isRouteExcluded(this.resolvedExclusions, pathname, RequestMethod[method]);
    }
}
function toRouteInfo(route) {
    ...
    if (!route ||
        ...
        throw new Error(`Invalid CSRF protection exclusion ${JSON.stringify(route)}: expected ` +
 ...
 */
function compileExclusion(path, requestMethod, logs) {
    const converted = LegacyRouteConverter.tryConvert(path, { logs });
    const normalized = converted.length > 1 ? converted.replace(/\/+$/, '') : converted;
    return {
        path: normalized,
        requestMethod,
