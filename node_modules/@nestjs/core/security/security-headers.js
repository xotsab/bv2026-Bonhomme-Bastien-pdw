 ...
 */
export const DEFAULT_CONTENT_SECURITY_POLICY_DIRECTIVES = Object.freeze({
    'default-src': ["'self'"],
    'base-uri': ["'self'"],
 ...
 */
const HELMET_LEGACY_OPTION_NAMES = {
 ...
 */
const MUST_BE_QUOTED = /^(?:self|none|strict-dynamic|report-sample|inline-speculation-rules|unsafe-inline|unsafe-eval|unsafe-hashes|wasm-unsafe-eval|unsafe-allow-redirects)$|^(?:nonce|sha256|sha384|sha512)-[A-Za-z0-9+/_=-]+$/i;
 ...
 */
export function serializeContentSecurityPolicy(options = {}) {
    ...
    if (useDefaults) {
        for (const [name, value] of Object.entries(DEFAULT_CONTENT_SECURITY_POLICY_DIRECTIVES)) {
    ...
    if (!result.has('default-src') && !removed.has('default-src')) {
        fail('Content-Security-Policy needs a "default-src" directive. Set ' +
            '"defaultSrc: null" to omit it deliberately.');
    }
    if (result.size === 0) {
        fail('Content-Security-Policy has no directives.');
    }
    return Array.from(result, ([name, values]) => values.length > 0 ? `${name} ${values.join(' ')}` : name).join(';');
}
function normalizeDirectiveName(rawName) {
    ...
    if (!NORMALIZED_DIRECTIVE_NAME.test(name)) {
        fail(`"${rawName}" is not a valid Content-Security-Policy directive name ` +
            '(expected camelCase, e.g. "scriptSrc", or lower-case kebab-case, ' +
            'e.g. "script-src").');
    }
    return name;
}
function normalizeDirectiveValue(name, value) {
    ...
    if (!Array.isArray(values)) {
        return fail(`Content-Security-Policy directive "${name}" must be a string, a ` +
            'list of strings, a boolean or null.');
    }
    return values.map(item => {
        if (!isString(item) || item.trim().length === 0) {
            return fail(`Content-Security-Policy directive "${name}" contains an empty or ` +
                'non-string value.');
        }
        if (FORBIDDEN_VALUE_CHARACTERS.test(item)) {
            return fail(`Content-Security-Policy directive "${name}" contains an invalid ` +
                `value ${JSON.stringify(item)}: ";", "," and control or non-ASCII ` +
                'characters are not allowed.');
        }
        if (item.split(/\s+/).some(token => MUST_BE_QUOTED.test(token))) {
            return fail(`Content-Security-Policy directive "${name}" contains ` +
...
export function resolveSecurityHeaders(options = {}) {
    ...
    if (csp) {
        add(readBoolean(csp, 'contentSecurityPolicy', 'reportOnly', false)
            ? 'Content-Security-Policy-Report-Only'
            : 'Content-Security-Policy', serializeContentSecurityPolicy(csp));
    }
    const coep = readOption(options, 'crossOriginEmbedderPolicy', false);
    if (coep) {
        add('Cross-Origin-Embedder-Policy', pick('crossOriginEmbedderPolicy.policy', coep.policy, COEP_POLICIES, 'require-corp'));
    }
    const coop = readOption(options, 'crossOriginOpenerPolicy', true);
    if (coop) {
        add('Cross-Origin-Opener-Policy', pick('crossOriginOpenerPolicy.policy', coop.policy, COOP_POLICIES, 'same-origin'));
    }
    const corp = readOption(options, 'crossOriginResourcePolicy', true);
    if (corp) {
        add('Cross-Origin-Resource-Policy', pick('crossOriginResourcePolicy.policy', corp.policy, CORP_POLICIES, 'same-origin'));
    }
    ...
    if (frameOptions) {
        ...
        add('X-Frame-Options', pick('xFrameOptions.action', isString(action) ? action.toLowerCase() : action, X_FRAME_OPTIONS_ACTIONS, 'sameorigin').toUpperCase());
    }
    const crossDomain = readOption(options, 'xPermittedCrossDomainPolicies', true);
    if (crossDomain) {
        add('X-Permitted-Cross-Domain-Policies', pick('xPermittedCrossDomainPolicies.permittedPolicies', crossDomain.permittedPolicies, X_PERMITTED_CROSS_DOMAIN_POLICIES, 'none'));
    }
 ...
 */
function readOption(options, name, enabledByDefault) {
    ...
    if (keys.length === 0 || !isPlainObject(value)) {
        return fail(`${name} must be a boolean${keys.length > 0 ? ' or an object' : ''}, ` +
            `received ${JSON.stringify(value)}.`);
    }
    for (const key of Object.keys(value)) {
        if (!keys.includes(key)) {
            fail(`unknown option "${name}.${key}" (expected ` +
                `${keys.map(k => `"${k}"`).join(', ')}).`);
        }
    }
    return value;
}
function readBoolean(object, option, key, fallback = false) {
    const value = object[key];
    if (value === undefined) {
        return fallback;
    }
    if (typeof value !== 'boolean') {
        fail(`${option}.${key} must be a boolean, received ${JSON.stringify(value)}.`);
    }
    return value;
}
function pick(option, value, allowed, fallback) {
    if (value === undefined) {
        return fallback;
    }
    if (!isString(value) || !allowed.includes(value)) {
        fail(`${option} must be one of ${allowed.map(v => `"${v}"`).join(', ')}, ` +
            `received ${JSON.stringify(value)}.`);
    }
    return value;
}
function resolveReferrerPolicy(policy) {
    if (policy === undefined) {
        return 'no-referrer';
    }
    const tokens = isString(policy) ? [policy] : policy;
...
function resolveHsts(option) {
    ...
    if (typeof maxAge !== 'number' || !Number.isFinite(maxAge) || maxAge < 0) {
        fail('strictTransportSecurity.maxAge must be a non-negative number of ' +
            `seconds, received ${JSON.stringify(maxAge)}.`);
    }
    let value = `max-age=${Math.floor(maxAge)}`;
    if (readBoolean(option, 'strictTransportSecurity', 'includeSubDomains', true)) {
