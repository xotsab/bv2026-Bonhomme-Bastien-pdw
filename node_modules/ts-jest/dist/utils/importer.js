...
class Importer {
    ...
    typescript(why, which) {
        ...
        if (!hasRequiredCompilerApi) {
            throw new Error((0, messages_1.interpolate)(messages_1.Errors.TypeScriptCompilerApiUnavailable, {
                module: which,
    ...
    _import(why, moduleName, { alternatives = [], installTip = moduleName } = {}) {
        ...
        if (res?.exists) {
            ...
            throw new Error((0, messages_1.interpolate)(messages_1.Errors.LoadingModuleFailed, { module: res.given, error: res.error.message }));
        }
        // if it couldn't load, build a nice error message so the user can fix it by himself
        const msg = alternatives.length ? messages_1.Errors.UnableToLoadAnyModule : messages_1.Errors.UnableToLoadOneModule;
