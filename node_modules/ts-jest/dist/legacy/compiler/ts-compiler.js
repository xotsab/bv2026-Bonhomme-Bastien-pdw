...
const assertCompilerOptionsWithJestTransformMode = (compilerOptions, isEsmMode, logger) => {
    if (isEsmMode && compilerOptions.module === typescript_1.default.ModuleKind.CommonJS) {
        logger.error(messages_1.Errors.InvalidModuleKindForEsm);
    }
};
class TsCompiler {
    configSet;
    ...
    getCompiledOutput(fileContent, fileName, options) {
        ...
        if (this._languageService) {
            if (constants_1.JS_JSX_REGEX.test(fileName) && !this._compilerOptions.allowJs) {
                this._logger.warn({ fileName: fileName }, (0, messages_1.interpolate)(messages_1.Errors.GotJsFileButAllowJsFalse, { path: fileName }));
                return {
                    code: fileContent,
                };
            }
            ...
            if (output.emitSkipped) {
                if (constants_1.TS_TSX_REGEX.test(fileName)) {
                    throw new Error((0, messages_1.interpolate)(messages_1.Errors.CannotProcessFile, { file: fileName }));
                }
                else {
                    this._logger.warn((0, messages_1.interpolate)(messages_1.Errors.CannotProcessFileReturnOriginal, { file: fileName }));
                    return {
                        code: fileContent,
                    };
                }
            ...
            if (!output.outputFiles.length) {
                throw new TypeError((0, messages_1.interpolate)(messages_1.Errors.UnableToRequireDefinitionFile, {
    ...
    _createLanguageService() {
        ...
        const serviceHost = {
            ...
            resolveModuleNames: (moduleNames, containingFile) => {
                ...
                return moduleNames.map((moduleName) => this._resolveModuleName(moduleName, containingFile, resolutionMode).resolvedModule);
            },
        };
     ...
     * @internal
     */
    _getResolutionMode(containingFile) {
        ...
        return getImpliedNodeFormat(containingFile, this._moduleResolutionCache?.getPackageJsonInfoCache?.(), 
        // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
     ...
     * @internal
     */
    _resolveModuleName(moduleNameToResolve, containingFile, resolutionMode) {
