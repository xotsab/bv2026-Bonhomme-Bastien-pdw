...
const migratePresetToTransformConfig = (transformConfig, preset, globalsTsJestConfig) => {
    if (preset) {
        const transformConfigFromPreset = preset.name === presets_1.JestPresetNames.jsWithTs
            ? (0, create_jest_preset_1.createJsWithTsPreset)(globalsTsJestConfig)
            : preset.name === presets_1.JestPresetNames.jsWIthBabel
...
const run = async (args /* , logger: Logger*/) => {
    ...
    if (migratedConfig.preset) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        preset = presets_1.allPresets[migratedConfig.preset] ?? presets_1.allPresets[presets_1.JestPresetNames.default];
    }
    else {
        if (args.js) {
            preset = args.js === 'babel' ? presets_1.allPresets[presets_1.JestPresetNames.jsWIthBabel] : presets_1.allPresets[presets_1.JestPresetNames.jsWithTs];
        }
        else {
            preset = presets_1.allPresets[presets_1.JestPresetNames.default];
        }
    }
